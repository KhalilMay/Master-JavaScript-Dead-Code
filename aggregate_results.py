...
if __name__ == '__main__':
    path = sys.argv[1]#"android-runner-master/examples/batterystats/output/2020.05.30_055428/data/j7duo/"
    level0_Sites, level1_Sites, level2_Sites, level3_Sites = explore_subjects(path)
    eng_headers = ['Optimization Levels', 'Website'] + ['Run {}'.format(x) for x in range(1, 21)] + ['Average']
    memory_headers =  ['Optimization Levels', 'Website'] + ['Run {}'.format(x) for x in range(1, 21)] + ['Average']
    cpu_headers = ['Optimization Levels', 'Website'] + ['Run {}'.format(x) for x in range(1, 21)] + ['Average']
    ...
    for CSS_Size, contents in level0_Sites.items():
        for _, path in contents.items():
            res1, res2, res3 = aggregate_subject(path, 19)
