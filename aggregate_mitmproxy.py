...
def aggregate_Metrics(j, fPath):#This function is called for each row of the aggregated_start csv files
    ...
    for x in header:
       for flow in os.listdir(fPath):
           ...
           data = data[data.Host == '192.168.178.178'] #Filter the data to have only the packets from the machine serving the subjects
           #print(j[x]/float(1000))
           data = data[(data.Request_timestamp_start > (j[x]/1000.0)) & (data.Request_timestamp_start < ((j[x]/1000.0)+120))] #j[x]/1000.0 is used because values collected from the onLoad event are in (ms) while values in request_timestamp_start are in seconds. (j[x]/1000.0) + 120 used because we need to collect the packets from the start of run to end of the run (some packets are recieved after the page is loaded so we use 120 seconds because each experiment run takes 2 minutes which is equal to 120 seconds)
           if len(data) != 0:
               if rotation == 0:
                   print(j[x])
                   print(len(data))
...
#CHECK THIS FOR COUNTING THE NUMBER OF ROWS IN THE TABLE AFTER FILTERING OUT WHAT WE WANT https://kite.com/python/answers/how-to-count-the-number-of-rows-in-a-pandas-dataframe-in-python
    ...
    level0_packets.to_csv('Aggregate_Metrics3/level0_Results_Packets.csv', header=True, index=None)
    level0_bytes.to_csv('Aggregate_Metrics3/level0_Results_Bytes.csv', header=True, index=None)

    level1_packets.to_csv('Aggregate_Metrics3/level1_Results_Packets.csv', header=True, index=None)
    level1_bytes.to_csv('Aggregate_Metrics3/level1_Results_Bytes.csv', header=True, index=None)

    level2_packets.to_csv('Aggregate_Metrics3/level2_Results_Packets.csv', header=True, index=None)
    level2_bytes.to_csv('Aggregate_Metrics3/level2_Results_Bytes.csv', header=True, index=None)

    level3_packets.to_csv('Aggregate_Metrics3/level3_Results_Packets.csv', header=True, index=None)
    level3_bytes.to_csv('Aggregate_Metrics3/level3_Results_Bytes.csv', header=True, index=None)
